...
//     const displayBook = (e: React.MouseEvent<HTMLDivElement>) => {
...
//     return (
//         <div id="book-card" className="book-card" onClick={displayBook}>
//             <img className="book-card-cover" src={book.cover} alt="Book Cover" />
//             <div className="book-card-info">
//                 <h1 className="book-card-title">{book.title}</h1>
//                 <h3 className="book-card-author">{mapAuthorsToString(book)}</h3>
//                 <p className="book-card-description">{book.description}</p>
...
export const BookCard: React.FC<BookCardProps> = ({ book }) => {
    ...
    useEffect(() => {
        if (book.records.length === 0) {
            ...
            setAvailable(book.records[0].status === 'AVAILABLE');
        }
    }, [book.records]);
    ...
    useEffect(() => {
        ...
        if (user?.type === 'EMPLOYEE' && available) {
            c += " checkout";
        } else if (user?.type === 'EMPLOYEE' && !available) {
            c += " checkin";
        }
    ...
    const displayBook = (e: React.MouseEvent<HTMLDivElement>) => {
        if ((e.target as HTMLElement).closest("button")) return; // Prevent navigation if clicking button
        navigate(`/resource/${book.barcode}`);
    };
